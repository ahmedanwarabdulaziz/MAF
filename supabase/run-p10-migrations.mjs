...
async function main() {
  console.log('=== P10 Migration Runner: Subcontractor Certificates ===\n')
  const test = await fetch(`https://api.supabase.com/v1/projects/${PROJECT_REF}`, {
    headers: { Authorization: `Bearer ${PAT}` }
  })
  if (!test.ok) {
    console.error('Cannot reach Supabase API:', test.status)
    process.exit(1)
  }
  ...
  const files = [
    'migrations/013_subcontractor_certificates.sql',
  ]

  for (const file of files) {
    await runSQL(file, readFileSync(join(__dirname, file), 'utf8'))
  }
  console.log('\n✅ P10 migrations complete.')
}
