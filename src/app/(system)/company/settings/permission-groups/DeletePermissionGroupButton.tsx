...
export default function DeletePermissionGroupButton({ groupId, groupName, isSystemGroup }: Props) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        const result = await deletePermissionGroupAction(groupId)
        if (result.error) {
          setError(result.error)
        } else {
          setShowConfirm(false)
          window.location.href = '/company/settings/permission-groups'
        }
      } catch (err) {
  ...
  return (
    <>
      <button
        ...
        title={isSystemGroup ? 'لا يمكن حذف مجموعات النظام' : 'حذف'}
        disabled={isSystemGroup}
        className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-text-secondary hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
      >
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-sm rounded-2xl border border-border bg-white shadow-2xl p-6">
              ...
              <h3 className="text-center text-lg font-bold text-text-primary mb-2">حذف مجموعة الصلاحيات</h3>
              <p className="text-center text-sm text-text-secondary mb-1">هل أنت متأكد من حذف المجموعة</p>
              <p className="text-center text-sm font-semibold text-text-primary mb-4">&quot;{groupName}&quot;؟</p>
              <p className="text-center text-xs text-text-secondary mb-6">
                سيتم حذف المجموعة وجميع الصلاحيات المرتبطة بها بشكل نهائي.
              </p>
