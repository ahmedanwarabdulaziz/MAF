...
import VendorStatementModal from '@/app/(system)/company/purchases/suppliers/VendorStatementModal'
...
export default function ExecutePaymentDialog({ 
  ...
  const partyId = partyRecord?.party_id || voucher.parties?.[0]?.party_id || null
  const partyName = partyRecord?.party?.arabic_name || voucher.parties?.[0]?.party?.arabic_name || 'غير معروف'
  ...
  return (
    <>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {partyId && (
                          <button
                            type="button"
                            onClick={() => setShowVendorStatement(true)}
                            title="عرض كشف حساب المورد التفصيلي"
                            className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-navy/10 hover:bg-navy/20 text-navy transition-colors shrink-0"
                          >
      ...
      {/* Vendor Statement Modal — reference view for payment approver */}
      {showVendorStatement && partyId && (
