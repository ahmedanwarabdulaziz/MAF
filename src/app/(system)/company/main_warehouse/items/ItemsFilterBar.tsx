...
export default function ItemsFilterBar({ groups }: { groups: Group[] }) {
  ...
  function pushParams(params: { q?: string; group?: string; status?: string }) {
  ...
  function clearAll() {
    setText(''); setGroupId(''); setStatus('')
    startTransition(() => router.push(pathname))
  }

  const hasFilters = text || groupId || status

  // Build group options: roots then children indented
  const roots = groups.filter(g => !g.parent_group_id)
  const childMap = groups.reduce<Record<string, Group[]>>((acc, g) => {
    if (g.parent_group_id) acc[g.parent_group_id] = [...(acc[g.parent_group_id] ?? []), g]
    return acc
  }, {})
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select
            ...
            className={`h-10 rounded-lg border px-3 pr-8 text-sm transition-colors focus:outline-none focus:ring-1 focus:ring-primary appearance-none ${
              groupId ? 'border-primary bg-primary/5 text-primary font-medium' : 'border-border bg-white text-text-secondary'
            }`}
            style={{ minWidth: 160 }}
          >
            <option value="">كل المجموعات</option>
            {roots.map(root => (
              <optgroup key={root.id} label={`📁 ${root.arabic_name}`}>
                <option value={root.id}>{root.group_code} – {root.arabic_name}</option>
                {(childMap[root.id] ?? []).map(child => (
                  <option key={child.id} value={child.id}>
                    &nbsp;&nbsp;↳ {child.group_code} – {child.arabic_name}
                  </option>
                ))}
              </optgroup>
            ))}
