...
import { createCompanyPurchaseInvoice } from '../actions'
...
export default function PurchaseInvoiceForm({ categories, suppliers, warehouses, items }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await createCompanyPurchaseInvoice({
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl">
                  ...
                  {invoiceType === 'general_expense' && (
                    <td className="py-2 pl-2">
                      <select
                        ...
                        onChange={e => updateLine(line.id, 'expense_category_id', e.target.value || null)}
                        className="border rounded px-2 py-1 text-xs w-40 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      >
                    ...
                    <td className="py-2 pl-2">
                      <select
                        ...
                        onChange={e => {
                          ...
                          updateLine(line.id, 'item_id', e.target.value || null)
                          if (item && !line.description) updateLine(line.id, 'description', item.arabic_name)
                        }}
                        className="border rounded px-2 py-1 text-xs w-44 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      >
                        <option value="">-- اختر صنف --</option>
                        {items.map(i => <option key={i.id} value={i.id}>{i.item_code} - {i.arabic_name}</option>)}
                      </select>
                    </td>
                  )}
                  <td className="py-2 pl-2">
      ...
      <div className="flex gap-3 justify-end">
        <button
          type="button"
          onClick={() => router.back()}
          className="px-5 py-2 rounded-lg border text-gray-700 hover:bg-gray-50 transition text-sm"
        >
          إلغاء
        </button>
        <button
          type="submit"
          disabled={isPending}
          className="px-6 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition text-sm font-medium"
        >
