...
import { createCompanyPurchaseReturn, postCompanyPurchaseReturn } from './actions'
import { useRouter } from 'next/navigation'

export default function CompanyPurchaseReturnDialog({ 
  invoice, 
  onClose,
  onSuccess 
}: { 
  ...
  const linesToReturn = invoice.lines.filter((l: any) => (l.quantity - (l.returned_quantity || 0)) > 0)

  linesToReturn.forEach((line: any) => {
    const qty = returnQtys[line.id] || 0
    if (qty > 0) {
      const lineNet = qty * Number(line.unit_price) // Note: simple net without tax/discount per line for now. If original lines had complex tax, we should proportion it.
      // the original system uses line_net = qty * unit_price usually, unless there are discounts.
      // We'll proportion tax and discount based on net amount.
      totalNet += lineNet
      totalGross += lineNet
    }
  })
  ...
  const handleSubmit = async () => {
    ...
    if (lines.length === 0) {
      alert('يجب تحديد كمية لمرتجع واحد على الأقل.')
      return
    }
    ...
    startTransition(async () => {
      try {
        ...
        const returnId = await createCompanyPurchaseReturn({
        ...
        await postCompanyPurchaseReturn(returnId)
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col rounded-2xl bg-white shadow-2xl overflow-hidden border border-border" dir="rtl">
        <div className="flex items-center justify-between border-b border-navy/10 bg-navy px-6 py-4 shrink-0 shadow-sm relative z-10">
          <div className="block w-full text-right">
            ...
            <p className="mt-1 text-sm text-white/80">
              قيمة المرتجع سيتم خصمها من رصيد المورد. 
              {invoice.invoice_type === 'stock_purchase' && ' (سيتم خصم الكميات من المخزون)'}
            </p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {linesToReturn.map((l: any) => {
                  const available = l.quantity - (l.returned_quantity || 0)
                  return (
                    <tr key={l.id} className="hover:bg-gray-50/50">
                      <td className="py-3 px-4 text-gray-900 font-medium">{l.description}</td>
                      <td className="py-3 px-4 text-gray-500">{fmt(l.quantity)}</td>
                      ...
                      <td className="py-2 px-4 bg-blue-50/30">
                        <input 
                          ...
                          className="w-full p-2 text-left rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                          placeholder="0.00"
                        />
                      </td>
                    </tr>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-white p-4 border border-border rounded-xl shadow-sm">
              ...
              <textarea 
                ...
                className="w-full p-3 rounded-xl border border-border focus:ring-2 focus:ring-primary shadow-sm"
                rows={4}
                placeholder="أسباب الارتجاع أو تفاصيل أخرى..."
              />
            </div>
        ...
        <div className="border-t border-navy/10 p-4 bg-gray-50 flex justify-end gap-3 shrink-0">
          ...
          <button
            type="button"
            className="px-6 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold disabled:opacity-50 transition shadow-sm"
