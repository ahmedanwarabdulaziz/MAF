  ...
  const { data: expense } = await supabase
    ...
    .select(`
      *,
      ...
      employee_custody_account:employee_custody_accounts(
         account_type, allowed_negative_limit,
         employee:users!employee_custody_accounts_employee_user_id_fkey(display_name, email)
      )
    `)
    .eq('id', params.doc_id)
    .single()
          ...
          <div className="rounded-xl border bg-card text-card-foreground shadow">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm font-medium text-muted-foreground mb-1">صاحب العهدة المسحوب منها</div>
                  <div className="font-semibold text-base">
                     {expense.employee_custody_account?.employee?.display_name || expense.employee_custody_account?.employee?.email?.split('@')[0]}
                  </div>
                  <div className="text-xs text-muted-foreground">
                     {expense.employee_custody_account?.account_type === 'permanent' ? 'مستديمة' : 'مؤقتة'}
                  </div>
                </div>
                <div>
                ...
                <div>
                  <div className="text-sm font-medium text-muted-foreground mb-1">التبويب المالي</div>
                  <div className="font-medium">{expense.expense_group?.arabic_name || '-'} / {expense.expense_item?.arabic_name || '-'}</div>
                </div>
                <div>
