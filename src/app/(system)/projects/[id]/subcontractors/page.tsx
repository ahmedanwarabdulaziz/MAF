...
export default async function SubcontractorsStatementPage({ params }: { params: { id: string } }) {
  ...
  const overallRetention  = statements.reduce((s, row) => s + Number(row.total_retention || 0), 0)
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            موقف مقاولي الباطن المالي، الدفعات المستحقة، وضمان الأعمال المحتجز حتى تاريخه.
          </p>
        </div>
        <div className="flex gap-4 items-center text-left">
          <div className="bg-background-secondary px-4 py-2 rounded-lg">
            <span className="text-xs text-text-secondary mb-1 block">إجمالي ضمان أعمال المشروع</span>
            <span className="text-xl font-bold text-amber-600 dir-ltr inline-block">
              {overallRetention.toLocaleString()} ج.م
            </span>
          </div>
          <div className="bg-background-secondary px-4 py-2 rounded-lg">
            ...
            <table className="w-full text-right text-sm">
              <thead className="bg-background-secondary border-b border-border">
                <tr>
                  ...
                  <th className="px-5 py-4 font-semibold text-text-secondary text-amber-700">ضمان أعمال</th>
                ...
                {statements.map((st) => (
                  <tr key={st.subcontractor_party_id} className="hover:bg-background-secondary/40 transition-colors">
                    ...
                    <td className="px-5 py-4 font-semibold text-amber-600 dir-ltr text-right bg-amber-50/20">
                      {Number(st.total_retention || 0).toLocaleString()}
                    </td>
