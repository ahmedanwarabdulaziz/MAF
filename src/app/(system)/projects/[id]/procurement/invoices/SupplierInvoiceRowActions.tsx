...
import { getInvoiceDetails, saveInvoiceLines, submitInvoiceForReceipt, confirmReceipt, getSupplierReturns, deleteSupplierReturn, postSupplierReturn } from '@/actions/procurement'
...
export default function SupplierInvoiceRowActions({ inv: rowInv, projectId, canApprove, canWhApprove, confirmation }: SupplierInvoiceRowActionsProps) {
  ...
  function handleConfirmSignature(role: 'pm' | 'warehouse_manager', e?: React.MouseEvent) {
  ...
  async function executeConfirmSignature() {
    ...
    setSuccessMessage(null)
    try {
      await confirmReceipt(rowInv.id, projectId, role)
  ...
  return (
    <div className="flex items-center gap-2 justify-end">
      ...
      {isPendingReceipt && canWhApprove && confirmation?.warehouse_manager_status !== 'approved' && (
        <button
          onClick={(e) => handleConfirmSignature('warehouse_manager', e)}
      ...
      {isPendingReceipt && canApprove && confirmation?.pm_status !== 'approved' && (
        <button
          onClick={(e) => handleConfirmSignature('pm', e)}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="rounded-xl border border-danger/20 bg-red-50/50 p-5 shadow-sm flex flex-col justify-center">
                      <p className="text-xs font-semibold text-danger mb-1">صافي المرتجعات</p>
                      <p className="text-2xl font-black text-danger dir-ltr text-right">{Number(inv.returned_amount || 0).toLocaleString()} ج.م</p>
                    </div>
                    <div className="rounded-xl border border-border bg-white p-5 shadow-sm flex flex-col justify-center">
                      <p className="text-xs font-semibold text-text-secondary mb-1">الرصيد المتبقي (غير مسدد)</p>
                      <p className="text-2xl font-black text-danger dir-ltr text-right">{Number(inv.outstanding_amount).toLocaleString()} ج.م</p>
                    </div>
                  </div>
                    ...
                    <div className="overflow-x-auto hide-scrollbar">
                      <table className="w-full text-right text-sm whitespace-nowrap">
                        <thead className="bg-background-secondary border-b border-border">
                          <tr>
                            <th className="font-semibold px-4 py-3 border-b text-navy">الكود</th>
                            <th className="font-semibold px-4 py-3 border-b text-navy">الصنف</th>
                            <th className="font-semibold px-4 py-3 border-b text-navy w-[12%]">الكمية المسعرة</th>
                            <th className="font-semibold px-4 py-3 border-b text-navy w-[12%]">الكمية الواردة (Receipt)</th>
