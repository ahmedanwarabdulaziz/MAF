            ...
            <div className="flex-1 overflow-y-auto p-6 bg-background-secondary/30 text-right" dir="rtl">
              {loading ? (
                ...
                <div className="space-y-6">
                  ...
                  {/* Attachments */}
                  {voucher.attachment_urls && voucher.attachment_urls.length > 0 && (
                    <div className="rounded-xl border border-border bg-white shadow-sm p-6 overflow-hidden">
                      <h2 className="text-lg font-bold text-navy border-b border-border pb-3 mb-4 flex items-center gap-2">
                        <span>إيصالات ومرفقات الدفع</span>
                        <span className="bg-primary/10 text-primary text-xs font-bold px-2 py-0.5 rounded-full">{voucher.attachment_urls.length}</span>
                      </h2>
                      <div className="flex flex-wrap gap-3">
                        {voucher.attachment_urls.map((url: string, idx: number) => (
                          <a key={idx} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-background-secondary hover:bg-black/5 border border-border rounded-lg text-sm font-medium text-navy transition-colors">
                            ...
                            <span dir="ltr">مرفق الشيك / الحوالة #{idx + 1}</span>
                          </a>
                        ))}
                      </div>
                    </div>
